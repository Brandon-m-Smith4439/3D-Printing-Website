...
function costingFromSnapshot(snapshot:QuoteCostSnapshot|null,settings:PricingSettings):QuoteCostInput{return snapshot?{materialLines:snapshot.materialLines.map(line=>({id:line.id,catalogItemId:line.catalogItemId,grams:line.grams})),machineHours:snapshot.machineHours,designHours:snapshot.designHours,laborHours:snapshot.laborHours,postProcessingHours:snapshot.postProcessingHours,packagingCostCents:snapshot.packagingCostCents,localDeliveryInternalCostCents:snapshot.localDeliveryInternalCostCents,miscellaneousCostCents:snapshot.miscellaneousCostCents}:{materialLines:[],machineHours:0,designHours:0,laborHours:0,postProcessingHours:0,packagingCostCents:settings.defaultPackagingCostCents,localDeliveryInternalCostCents:0,miscellaneousCostCents:0};}
    ...
    <section className="quote-workspace" role="dialog" aria-modal="true" aria-label={`Quote workspace for ${request.requestCode}`}>
      ...
      <div className="quote-workspace-layout">
        <div className="quote-workspace-main">
          ...
          <OwnerQuoteCostPanel settings={pricing.settings} catalog={pricing.catalog} materialCosts={pricing.materialCosts} value={costing} customerTotalCents={pricingCustomerTotalCents} shippingInternalCostCents={pricingShippingInternalCents} disabled={locked||busy} onChange={setCosting} onUseSuggestedPrice={(suggestedTotal)=>{const nextBase=Math.max(0,suggestedTotal-assemblyFeeCents-rushFeeCents-localDeliveryFeeCents-activeShippingCustomerCents);setBasePrice(dollars(nextBase));}}/>
