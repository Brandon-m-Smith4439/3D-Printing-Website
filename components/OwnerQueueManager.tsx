...
function ProductionPanel({pricing,requests,jobs,quotes,shipments,finalInvoices,pickups,followUps,focusedRequestId,onFocusedRequestHandled,onChanged,onNotice}:{pricing:OwnerPricingPayload;requests:StoredRequest[];jobs:QueueJob[];quotes:StoredQuote[];shipments:ShipmentRecord[];finalInvoices:FinalInvoiceRecord[];pickups:PickupAppointment[];followUps:Record<string,OwnerRequestFollowUpView>;focusedRequestId:string;onFocusedRequestHandled:()=>void;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  return <>
    ...
    <div className="production-request-list">{visible.length===0?<div className="queue-empty compact"><strong>No matching requests.</strong></div>:visible.map(r=><CombinedRequestCard key={r.id} request={r} quote={quotes.find(q=>q.requestId===r.id&&q.status!=="void")||null} shipment={shipments.find(s=>s.requestId===r.id)||null} finalInvoice={finalInvoices.filter(i=>i.requestId===r.id).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))[0]||null} pickup={pickups.find(p=>p.requestId===r.id&&p.status==="scheduled")||null} job={r.queueJobId?jobs.find(j=>j.id===r.queueJobId)||null:null} allJobs={jobs} activeCount={jobs.filter(j=>j.status!=="completed").length} followUp={followUps[r.id]||null} pricing={{settings:pricing.settings,catalog:pricing.catalog,materialCosts:pricing.materialCosts,snapshots:pricing.costing[r.id]||[]}} forceOpen={focusedRequestId===r.id} onChanged={onChanged} onNotice={onNotice}/>)}</div>
    </section>
...
function CombinedRequestCard({request,quote,shipment,finalInvoice,pickup,job,allJobs,activeCount,followUp,pricing,forceOpen,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote|null;shipment:ShipmentRecord|null;finalInvoice:FinalInvoiceRecord|null;pickup:PickupAppointment|null;job:QueueJob|null;allJobs:QueueJob[];activeCount:number;followUp:OwnerRequestFollowUpView|null;pricing:{settings:PricingSettings;catalog:BambuFilamentCatalogItem[];materialCosts:Record<string,ResolvedMaterialCost>;snapshots:QuoteCostSnapshot[]}|null;forceOpen:boolean;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  const release=evaluateFulfillmentRelease({
    ...
    depositSatisfied:depositActuallyPaid,
    jobStatus:job?.status||"",
    finalBalancePaid:balancePaid,
  ...
  return <article id={`owner-request-${request.id}`} className={`production-request-card ${request.riskLevel==="review"?"owner-request-risk":""}`}>
    ...
    {open&&<div className="production-request-body owner-request-workspace">
      ...
      {quote&&job&&<section className="owner-fulfillment-release">
        <div className="owner-fulfillment-release-heading"><div><span className="eyebrow">FULFILLMENT RELEASE</span><strong>{release.canComplete?"Order can be completed":"Resolve release checks before completion"}</strong></div><em className={release.canComplete?"is-ready":"is-blocked"}>{release.canComplete?"Release ready":"Guarded"}</em></div>
        <div className="owner-fulfillment-release-grid">{release.checks.map(check=><article className={`status-${check.status}`} key={check.id}><span>{check.status==="ready"?"✓":check.status==="blocked"?"!":"•"}</span><div><strong>{check.label}</strong><small>{check.detail}</small></div></article>)}</div>
      </section>}
      ...
      {quote?.fulfillmentMode==="pickup"&&job&&(["ready","completed"].includes(job.status)||pickup)&&<section className="owner-pickup-appointment">
        ...
        {pickup?<><div className="owner-pickup-facts"><span><b>Date</b>{new Date(`${pickup.slotDate}T12:00:00`).toLocaleDateString()}</span><span><b>Time</b>{new Intl.DateTimeFormat("en-US",{hour:"numeric",minute:"2-digit"}).format(new Date(`2000-01-01T${pickup.slotTime}:00`))}</span><span><b>Location</b>{pickup.location.locationName}</span><span><b>Customer</b>{request.name}</span></div><p>{[pickup.location.street1,pickup.location.street2,[pickup.location.city,pickup.location.state,pickup.location.zip].filter(Boolean).join(", ")].filter(Boolean).join(" · ")}</p>{pickup.location.instructions&&<small>{pickup.location.instructions}</small>}</>:<p>The order is Ready. The customer can reserve one of the available times from their profile after pickup scheduling is enabled and the location is configured under Site Content.</p>}
      </section>}
...
function OwnerShipmentPanel({request,quote,shipment,finalInvoice,job,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote;shipment:ShipmentRecord|null;finalInvoice:FinalInvoiceRecord|null;job:QueueJob|null;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  const release=evaluateFulfillmentRelease({
    ...
    depositSatisfied:depositActuallyPaid(quote),
    jobStatus:job?.status||"",
    finalBalancePaid:finalInvoicePaid(finalInvoice),
    shippingSelected:Boolean(selected),
    trackingCode:shipment?.trackingCode||"",
    shipmentStatus:shipment?.status||"not_created",
    refundStatus:shipment?.refundStatus||"",
  ...
  async function buy(force=false){
    if(!selected)return;
    if(!job||job.status!=="ready"){onNotice({kind:"warning",text:"Mark the production job Ready before purchasing its shipping label."});return;}
    const destination=`${selected.address.city}, ${selected.address.state} ${selected.address.zip}`;
    const confirmed=force
      ? window.confirm(`Current postage changed to ${money(shipment?.proposedRateCents||0)}. Authorize this ${selected.carrier} ${selected.service} label purchase to ${destination} anyway? The customer quote will not be changed automatically.`)
      : window.confirm(`Authorize purchase of the ${selected.carrier} ${selected.service} shipping label for ${request.requestCode} to ${destination}? EasyPost will re-rate the shipment before purchase.`);
    if(!confirmed)return;
    setBusy(true);
    try{
      const response=await fetch(`/api/owner/requests/${request.id}/shipping-label`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({confirmPurchase:true,force,confirmRateChange:force})});
