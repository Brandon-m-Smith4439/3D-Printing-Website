...
export function OwnerQueueManager(){
  ...
  return <div className="owner-dashboard">
    ...
    {tab==="production"&&<ProductionPanel requests={requests} jobs={jobs} quotes={quotes} shipments={shipments} finalInvoices={finalInvoices} onChanged={loadAll} onNotice={setNotice}/>}  {tab==="site"&&siteContent&&<SiteContentPanel content={siteContent} onChanged={loadAll} onNotice={setNotice}/>} {tab==="security"&&<OwnerSecurityPanel onNotice={setNotice}/>} 
  </div>;
}

function ProductionPanel({requests,jobs,quotes,shipments,finalInvoices,onChanged,onNotice}:{requests:StoredRequest[];jobs:QueueJob[];quotes:StoredQuote[];shipments:ShipmentRecord[];finalInvoices:FinalInvoiceRecord[];onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  return <>
    ...
    <div className="production-request-list">{visible.length===0?<div className="queue-empty compact"><strong>No matching requests.</strong></div>:visible.map(r=><CombinedRequestCard key={r.id} request={r} quote={quotes.find(q=>q.requestId===r.id&&q.status!=="void")||null} shipment={shipments.find(s=>s.requestId===r.id)||null} finalInvoice={finalInvoices.filter(i=>i.requestId===r.id).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))[0]||null} job={r.queueJobId?jobs.find(j=>j.id===r.queueJobId)||null:null} allJobs={jobs} activeCount={jobs.filter(j=>j.status!=="completed").length} onChanged={onChanged} onNotice={onNotice}/>)}</div>
    </section>
...
function CombinedRequestCard({request,quote,shipment,job,allJobs,activeCount,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote|null;shipment:ShipmentRecord|null;job:QueueJob|null;allJobs:QueueJob[];activeCount:number;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  const liveStatus=workflowStatus(request,quote,job); const priority=calculateRequestPriority(request); const paidRecord=Boolean(quote&&(quote.payments.length>0||quote.depositPaidAt))||request.status==="completed"; const depositActuallyPaid=Boolean(quote&&quote.status==="deposit-paid"&&quoteDepositSatisfied(quote));
  ...
  async function queueStatus(status:QueueStatus){if(!job)return;if(status==="completed"&&job.status!=="completed"&&!window.confirm(job.customerEmail?`Mark ${job.publicCode} completed and email ${job.customerEmail}?`:`Mark ${job.publicCode} completed? No customer email is stored for this request.`))return;setBusy(true);try{const response=await fetch(`/api/owner/queue/${job.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status})});const result=await response.json() as {message?:string;emailWarning?:string;shippingWarning?:string};if(!response.ok)throw new Error(result.message||"Could not update production status.");const warning=result.emailWarning||result.shippingWarning||"";onNotice({kind:warning?"warning":"success",text:warning||"Production status updated."});await onChanged();}catch(e){onNotice({kind:"error",text:e instanceof Error?e.message:"Could not update status."});}finally{setBusy(false);}}
  ...
  return <article className={`production-request-card ${request.riskLevel==="review"?"owner-request-risk":""}`}>
    ...
    {open&&<div className="production-request-body owner-request-workspace">
      ...
      {quoteOpen&&<OwnerQuoteEditor request={request} quote={quote} jobs={allJobs} onChanged={onChanged} onNotice={onNotice} onClose={()=>setQuoteOpen(false)}/>} 
    </div>}
  </article>;
}
