...
export function OwnerQueueManager(){
  ...
  return <div className="owner-dashboard">
    ...
    {tab==="production"&&<ProductionPanel requests={requests} jobs={jobs} quotes={quotes} shipments={shipments} finalInvoices={finalInvoices} focusedRequestId={focusedRequestId} onFocusedRequestHandled={()=>setFocusedRequestId("")} onChanged={loadAll} onNotice={setNotice}/>}
...
function ProductionPanel({requests,jobs,quotes,shipments,finalInvoices,focusedRequestId,onFocusedRequestHandled,onChanged,onNotice}:{requests:StoredRequest[];jobs:QueueJob[];quotes:StoredQuote[];shipments:ShipmentRecord[];finalInvoices:FinalInvoiceRecord[];focusedRequestId:string;onFocusedRequestHandled:()=>void;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  return <>
    ...
    <div className="production-request-list">{visible.length===0?<div className="queue-empty compact"><strong>No matching requests.</strong></div>:visible.map(r=><CombinedRequestCard key={r.id} request={r} quote={quotes.find(q=>q.requestId===r.id&&q.status!=="void")||null} shipment={shipments.find(s=>s.requestId===r.id)||null} finalInvoice={finalInvoices.filter(i=>i.requestId===r.id).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))[0]||null} job={r.queueJobId?jobs.find(j=>j.id===r.queueJobId)||null:null} allJobs={jobs} activeCount={jobs.filter(j=>j.status!=="completed").length} forceOpen={focusedRequestId===r.id} onChanged={onChanged} onNotice={onNotice}/>)}</div>
    </section>
...
function CombinedRequestCard({request,quote,shipment,finalInvoice,job,allJobs,activeCount,forceOpen,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote|null;shipment:ShipmentRecord|null;finalInvoice:FinalInvoiceRecord|null;job:QueueJob|null;allJobs:QueueJob[];activeCount:number;forceOpen:boolean;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
