...
export function OwnerQueueManager(){
  ...
  return <div className="owner-dashboard">
    <div className="owner-toolbar"><div className="owner-tabs"><button className={tab==="operations"?"is-active":""} onClick={()=>setTab("operations")} type="button">Dashboard</button><button className={tab==="production"?"is-active":""} onClick={()=>setTab("production")} type="button">Production {newCount>0&&<b>{newCount}</b>}</button><button className={tab==="site"?"is-active":""} onClick={()=>setTab("site")} type="button">Site Content</button><button className={tab==="security"?"is-active":""} onClick={()=>setTab("security")} type="button">Security & Backups</button></div><button className="button button-secondary button-small" onClick={logout} type="button">Sign out</button></div>
    {notice&&<div className={`owner-notice ${notice.kind}`}>{notice.text}</div>}
    {tab==="operations"&&<OwnerOperationsCenter onNotice={setNotice} onOpenRequest={(requestId)=>{setFocusedRequestId(requestId);setTab("production");}}/>}
    {tab==="production"&&<ProductionPanel requests={requests} jobs={jobs} quotes={quotes} shipments={shipments} finalInvoices={finalInvoices} followUps={followUps} focusedRequestId={focusedRequestId} onFocusedRequestHandled={()=>setFocusedRequestId("")} onChanged={loadAll} onNotice={setNotice}/>}
...
function ProductionPanel({requests,jobs,quotes,shipments,finalInvoices,followUps,focusedRequestId,onFocusedRequestHandled,onChanged,onNotice}:{requests:StoredRequest[];jobs:QueueJob[];quotes:StoredQuote[];shipments:ShipmentRecord[];finalInvoices:FinalInvoiceRecord[];followUps:Record<string,OwnerRequestFollowUpView>;focusedRequestId:string;onFocusedRequestHandled:()=>void;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  return <>
    ...
    <div className="production-request-list">{visible.length===0?<div className="queue-empty compact"><strong>No matching requests.</strong></div>:visible.map(r=><CombinedRequestCard key={r.id} request={r} quote={quotes.find(q=>q.requestId===r.id&&q.status!=="void")||null} shipment={shipments.find(s=>s.requestId===r.id)||null} finalInvoice={finalInvoices.filter(i=>i.requestId===r.id).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))[0]||null} job={r.queueJobId?jobs.find(j=>j.id===r.queueJobId)||null:null} allJobs={jobs} activeCount={jobs.filter(j=>j.status!=="completed").length} followUp={followUps[r.id]||null} forceOpen={focusedRequestId===r.id} onChanged={onChanged} onNotice={onNotice}/>)}</div>
    </section>
...
function CombinedRequestCard({request,quote,shipment,finalInvoice,job,allJobs,activeCount,followUp,forceOpen,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote|null;shipment:ShipmentRecord|null;finalInvoice:FinalInvoiceRecord|null;job:QueueJob|null;allJobs:QueueJob[];activeCount:number;followUp:OwnerRequestFollowUpView|null;forceOpen:boolean;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
