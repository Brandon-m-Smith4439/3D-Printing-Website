...
function CombinedRequestCard({request,quote,shipment,job,allJobs,activeCount,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote|null;shipment:ShipmentRecord|null;job:QueueJob|null;allJobs:QueueJob[];activeCount:number;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  async function queueStatus(status:QueueStatus){if(!job)return;if(status==="completed"&&job.status!=="completed"&&!window.confirm(`Mark ${job.publicCode} completed and email ${job.customerEmail}?`))return;setBusy(true);try{const response=await fetch(`/api/owner/queue/${job.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status})});const result=await response.json() as {message?:string;emailWarning?:string;shippingWarning?:string};if(!response.ok)throw new Error(result.message||"Could not update production status.");const warning=result.emailWarning||result.shippingWarning||"";onNotice({kind:warning?"warning":"success",text:warning||"Production status updated."});await onChanged();}catch(e){onNotice({kind:"error",text:e instanceof Error?e.message:"Could not update status."});}finally{setBusy(false);}}
