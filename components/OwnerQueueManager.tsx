...
function workflowStatus(request:StoredRequest,quote:StoredQuote|null,job:QueueJob|null){
  ...
  if(quote?.depositPaidAt||quote?.status==="deposit-paid")return "Deposit paid — ready to schedule";
...
function CombinedRequestCard({request,quote,job,activeCount,onChanged,onNotice}:{request:StoredRequest;quote:StoredQuote|null;job:QueueJob|null;activeCount:number;onChanged:()=>Promise<void>;onNotice:(n:Notice)=>void}){
  ...
  return <article className={`production-request-card ${request.riskLevel==="review"?"owner-request-risk":""}`}>
    <div className="production-request-header" role="button" tabIndex={0} aria-expanded={open} onClick={toggle} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();toggle();}}}>
      ...
      <div className="production-status-select" onClick={e=>e.stopPropagation()} onKeyDown={e=>e.stopPropagation()}>{job?<select aria-label="Production status" value={job.status} disabled={busy} onChange={e=>void queueStatus(e.target.value as QueueStatus)}>{Object.entries(queueLabels).map(([v,l])=><option value={v} key={v}>{l}</option>)}</select>:(["new","reviewing"].includes(request.status)&&(!quote||quote.status==="draft"))?<select aria-label="Request status" value={request.status} disabled={busy} onChange={e=>void requestStatus(e.target.value)}><option value="new">New</option><option value="reviewing">Reviewing</option><option value="declined">Declined</option></select>:<span className={`owner-lifecycle-status status-${quote?.status||request.status}`}>{liveStatus}</span>}</div>
    </div>
    {open&&<div className="production-request-body owner-request-workspace">
      ...
      {job?<LinkedQueueEditor job={job} position={position} setPosition={setPosition} activeCount={activeCount} move={move} moveTo={moveTo} onChanged={onChanged} onNotice={onNotice}/>:<div className="production-not-queued"><strong>{request.status==="deposit-paid"?"Deposit confirmed — ready to schedule.":quote?.status==="approved"?"Quote approved — waiting for deposit.":quote?.status==="sent"?"Quote sent — waiting for customer response.":quote?.status==="countered"?"Customer sent a counter offer.":quote?.status==="declined"?"Customer declined the current quote.":"Not in production queue."}</strong><span>{request.status==="deposit-paid"?"Add it to production when ready; it will go to the end of the queue.":quote?.status==="countered"||quote?.status==="declined"?"Open the Quote Workspace to review the history and send a revised quote if appropriate.":"Production should begin only after quote approval and the 50% deposit."}</span>{request.status==="deposit-paid"&&<button className="button button-small" type="button" disabled={busy} onClick={()=>void addToQueue()}>Add to Production Queue</button>}</div>}
