...
import type { QuoteHistoryEntry, QuoteStatus } from "@/lib/quote-types";
...
function displayedStatus(request:ProfileRequest){if(request.queue)return queueLabels[request.queue.status];if(request.quote)return quoteLabels[request.quote.status];return requestLabels[request.status];}
...
export function ProfileDashboard({customer}:Props){
  ...
  async function load(){const response=await fetch("/api/account/requests",{cache:"no-store"});if(response.status===401){router.push("/login");return;}const result=await response.json() as {requests?:ProfileRequest[];notifications?:CustomerNotification[]};setRequests(result.requests||[]);setNotifications(result.notifications||[]);setLoading(false);}
    ...
    <section className="profile-section" id="requests"><div className="owner-panel-heading"><div><p className="eyebrow">YOUR REQUESTS</p><h2>Requests, quotes & queue status</h2></div><button className="text-button" type="button" onClick={()=>void load()}>Refresh</button></div>
      {loading?<div className="owner-loading">Loading your requests…</div>:requests.length===0?<div className="queue-empty"><strong>No profile-linked requests yet.</strong><span>Sign in before submitting a custom request and it will appear here automatically.</span></div>:<div className="profile-request-list">{requests.map(request=>{const title=request.queue?.publicTitle||"Custom 3D print request";const status=displayedStatus(request);const q=request.quote;return <article className={`profile-request-card ${request.queue?.imageUrl?"has-image":""}`} key={request.id}>
        ...
        <div className="profile-request-main"><div className="profile-request-top"><div><span>{request.requestCode}</span><h3>{title}</h3><small>Submitted {new Date(request.createdAt).toLocaleDateString()}</small></div><span className={`request-status quote-status-${q?.status||request.status}`}>{status}</span></div><p>{request.description}</p>
          ...
          {q&&<section className="customer-quote-card"><div className="customer-quote-heading"><div><span className="eyebrow">QUOTE REVISION {q.revision}</span><h4>{quoteLabels[q.status]}</h4></div><strong>{money(q.totalCents)}</strong></div><div className="customer-quote-facts"><span><b>50% deposit</b>{money(q.depositCents)}</span><span><b>Remaining balance</b>{money(q.balanceCents)}</span><span><b>Material</b>{q.material}</span><span><b>Dimensions</b>{q.dimensions}</span></div>
            ...
            {q.status==="sent"&&<div className="customer-quote-response-actions"><button className="button button-small" disabled={!customer.emailVerified||busyQuote===q.id} onClick={()=>void approveQuote(q.id)} type="button">{busyQuote===q.id?"Working…":"Approve Quote & Terms"}</button><button className="button button-secondary button-small" disabled={!customer.emailVerified||busyQuote===q.id} onClick={()=>setCounterQuoteId(counterQuoteId===q.id?"":q.id)} type="button">Counter Offer</button><button className="text-button danger-text" disabled={!customer.emailVerified||busyQuote===q.id} onClick={()=>void respondQuote(q.id,"decline")} type="button">Decline Quote</button></div>}
